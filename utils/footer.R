...
footer_reactive <- function(all_games, client_timezone) {
    last_updated <- as.POSIXct(max(all_games$last_updated_et, na.rm = TRUE))
    ...
    last_updated_tz <- format(last_updated, " %Z")

    div(paste0("Data last refreshed: ", last_updated_date, " at ", last_updated_time, last_updated_tz, "."))
}
